...
class PCDPublisher(Node):
    def __init__(self):
        ...
        # ros2 run pcd_ros2_demo pcd_publisher_node /path/to/ply
        assert len(sys.argv) > 1, "No ply file given."
        assert os.path.exists(sys.argv[1]), "File doesn't exist."
        pcd_path = sys.argv[1]

        # use Open3D to read point clouds and meshes. It's a great library!
        pcd = o3d.io.read_point_cloud(pcd_path)
        # then convert it into a numpy array.
        self.points = np.asarray(pcd.points)
        print(self.points.shape)
 
        # create a publisher that publishes sensor_msgs.PointCloud2 to the 
